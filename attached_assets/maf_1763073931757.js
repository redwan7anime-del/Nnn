...
// قواعد   -> show rules
// idroom  -> show room id
// add room -> add current room
// rem room -> remove current room
// rem -> remove member
// add -> add member
...
const STR = {
  ...
  cancelledAct:  `↩️ تم تغيير اختيارك.`,
  ...
  loverAlreadyLinked: `⚠️ ربطت شخصين بالفعل — لا يمكنك التغيير.`,
  ...
  voteBlocked:   `⛓️ صوتك مُصادَر — السجان قيّدك.`,
  tie:           `تعادل في التصويت لا اعدام اليوم ⚖️`,
  gameOver:      (winner, reason, civLines, mafiaLines, summary) =>
`╔═══════ ☠️ ═══════╗
⛧ نــهـايــة الـمـذبـحـة ⛧
╚═══════ ☠️ ═══════╝
🩸 ⟡ المنتصرون: ${winner} ⡀ 🏆
⚰️ ⡀ السبب: ${reason} ☠️
...
const DEATH_LINES = [
  n => `${n} وُجد جثةً باردة — ولن يعود أبداً.`,
  n => `${n} ابتُلع بالظلام ولم يُبصر فجراً.`,
  n => `${n} صرخ أخيراً لكن لم يسمعه أحد.`,
  n => `${n} غادر بصمت مفروض عليه.`,
  n => `${n} كان هنا — ثم لم يكن.`,
  n => `${n} الرصاصة أو الحبل، النتيجة واحدة.`,
  n => `${n} دمه على أيدي من بقي.`,
];
...
function aliveDeadText(game){
  ...
  const aLines = alive.length
    ? alive.map(p=>`🍀 ${p.name}`).join('\n')
    : '『 لا أحد بقي 』';

  const dLines = dead.length
    ? dead.map(p=>`💀 ${p.name} — 『 ${p.role} 』`).join('\n')
    : '『 لا جثث حتى الآن 』';

  return [
    `━━━━━━━━━━━━━━━━`,
    `🌿 الأحياء — ${alive.length} لاعب`,
    `━━━━━━━━━━━━━━━━`,
    aLines,
    ``,
    `━━━━━━━━━━━━━━━━`,
    `💀 الضحايا — ${dead.length} لاعب`,
    `━━━━━━━━━━━━━━━━`,
    dLines
  ].join('\n');
}
...
const ROLE_DESCS = {
  ...
  'سجان':    '⛓️ قدرتك تقييد الآخرين — كل ليلة اختر لاعباً واحداً يُمنع من التصويت في اليوم التالي.\nاستهدف المشبوهين أو من تريد إسكاتهم.',
  ...
  'دراكولا': '🧛 إمبراطور الليل — كل ليلة حول شخصاً إلى مصاص دماء يفقد قدرته الأصلية.\nإذا أصبح عدد مصاصي الدماء أكثر من باقي اللاعبين، تفوز أنت وجميع المصاصين معك!',
...
function assignRoles(players){
  ...
  if(civs.length>=5) roles[civs[4]]='سجان';
...
async function resolveNight(game){
  ...
  for(const [uid,act] of Object.entries(game.actionQueue)){
    const a=byId[uid]; if(!a||!a.alive) continue;
    if(a.role==='سجان' && act.target) game.blockedVoters.push(act.target);
  }
  ...
  for(const d of dead){
    const p=byId[d]; if(p&&p.alive){
      ...
      if(lover) deaths.push({userID:lover.userID,name:lover.name,role:lover.role,byLover:true,partnerName:p.name});
    }
  }
...
function sendAsync(threadID, text){
  return sendMessageWithRetry({body:text}, threadID);
}
...
login(CREDENTIALS, (err, a)=>{
  ...
  function startGameInThread(threadID){
    ...
    setTimeout(()=> send(threadID, `⏳ لديكم ${totalSec} ثانية للانضمام — تفاعل على الصورة.`), 1500);

    // reminder at 60s remaining
    if(DEFAULTS.joinWindowMs > 65000){
      game.timers.remind60 = setTimeout(()=>{
        if(GAMES[id] && GAMES[id].phase==='joining')
          send(threadID, `⏰ تبقى 60 ثانية — ${Object.keys(game.joinReactions).length} لاعب سجّل تفاعله حتى الآن.`);
    ...
    if(DEFAULTS.joinWindowMs > 35000){
      game.timers.remind30 = setTimeout(()=>{
        if(GAMES[id] && GAMES[id].phase==='joining')
          send(threadID, `⏰ تبقى 30 ثانية — ${Object.keys(game.joinReactions).length} لاعب. سارع بالانضمام!`);
  ...
  async function finalizeJoining(gameId){
    ...
    if(reactorIDs.length < 3){
      send(game.threadMainID, `❌ لم يكتمل العدد (${reactorIDs.length} لاعبين). الحد الأدنى 3.`);
    ...
    send(game.threadMainID, `🎮 جاري إضافة ${reactorIDs.length} لاعب إلى الغرف...`);
  ...
  function nightActorsExpected(game){
    ...
    for(const p of game.players){
      if(!p.alive) continue;
      if(p.role === 'مواطن' || p.role === 'مهرج') continue;
  ...
  async function startNight(gameId){
    ...
    async function doNightResolution(){
      ...
      // Send دراكولا transformation notifications - تم الإرسال فوراً
      // if(g._draculaTransformations && g._draculaTransformations.length > 0){
      //   for(const t of g._draculaTransformations){
      //     await dm(t.transformedID, STR.draculaTransform(t.transformedName));
      //   }
      //   g._draculaTransformations = [];
      // }
      ...
      if(realDeaths.length===0){
        ...
        for(const d of deaths){
          let msg;
          if(d.byLover)    msg = STR.loverDeath(d.name, d.partnerName);
          else if(d.byScp) {
            g.scpKillCount++;
            msg = STR.scpKill(d.name, g.scpKillCount);
          }
          else if(d.bySniper && !d.wasted) msg = STR.sniperResult(d.name, d.role === 'مافيا');
          else if(d.bySniper && d.wasted) continue;
          else if(d.byMafia) msg = STR.mafiaDeath(d.name, d.role);
          // else             msg = STR.morningDeath(d.name, d.role);
          if(msg) await sendWait(g.threadMainID, msg, 3000);
        }
        // Revivals after all deaths announced (so laanCurseKill appears before laanRevived)
        for(const d of deaths){
          if(d.laanRevived) await sendWait(g.threadMainID, STR.laanRevived(d.name), 3000);
        }
      }
  ...
  async function startDay(gameId){
    ...
    for(const p of alive){
      try {
        if((game.blockedVoters || []).includes(p.userID)){
          await dm(p.userID, `⛓️ السجان قيّدك — صوتك مُصادَر هذا اليوم. لا يمكنك التصويت.`);
        } else {
    ...
    async function doDayResolution(){
      ...
      if(max >= 2){
        ...
        const statsLines = voted.map(p=>`${game.voteTally[p.userID]} — ${p.name}`).join('\n');
      ...
      if(max < 2){
        ...
        if(ex){
          ...
          if(ex.role === 'مهرج'){
            await sendWait(game.threadMainID, STR.voteResultJester(ex.name), 3000);
            saveGame(game);
          ...
          if(loverVote){
            await sendWait(game.threadMainID, STR.loverDeath(loverVote.name, ex.name), 3000);
          }
        }
      } else {
  ...
  const ROLE_EMOJIS = {
    ...
    'مهرج':'🃏','سجان':'⛓️','خطابة':'🧚','مواطن':'👤',
    'SCP-096':'☣️','نهاب':'🦹','دراكولا':'🧛','مصاص دماء':'🧛'
  };
  ...
  async function endGame(game, win){
    ...
    // Build civilian and mafia lines
    const civilians = game.players.filter(p => p.role !== 'مافيا');
    const mafia     = game.players.filter(p => p.role === 'مافيا');
    ...
    const civLines   = civilians.map(fmtPlayer).join('\n');
    const mafiaLines = mafia.map(fmtPlayer).join('\n');

    const mafiaAlive = mafia.filter(p=>p.alive).length;
    const civAlive   = civilians.filter(p=>p.alive).length;
    let summary;
    if(mafiaAlive===0 && civAlive===0) summary = '☠️ 💀 انقرض الجميع';
    else if(mafiaAlive===0)            summary = '☠️ 💀 لا ناجين من المافيا';
    else if(civAlive===0)              summary = '☠️ 💀 لا ناجين من المدنيين';
    else                               summary = `🩸 بقي ${civAlive} مدني و${mafiaAlive} من المافيا`;

    // Winner announcement
    await sendAsync(game.threadMainID, STR.gameOver(win.winner, win.reason, civLines, mafiaLines, summary));
            ...
            if(game.phase === 'night'){
              ...
              if(player.role === 'محقق'){
                ...
                if(target && target.role === 'مافيا'){
                  dm(sender, `⚠️ لا تستطيع تحويل المافيا! اختر هدفاً آخر.`);
                } else {
                  dm(sender, `🧛 ممتاز... سيتحول ${target ? target.name : 'الهدف'} إلى مصاص دماء في الصباح!`);
                }
              } else {
